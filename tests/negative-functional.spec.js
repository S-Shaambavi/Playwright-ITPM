...
async function typeAndConvert(page, text) {
  ...
  await inputBox.pressSequentially(text, { delay: 20 });
...
test.describe('Negative Functional Tests - Thanglish to Tamil Transliteration', () => {
  ...
  test('Neg_Fun_0001 - Joined words without spaces', async ({ page }, testInfo) => {
  ...
  test('Neg_Fun_0002 - English with numeric content', async ({ page }, testInfo) => {
  ...
  test('Neg_Fun_0003 - English long sentence', async ({ page }, testInfo) => {
  ...
  test('Neg_Fun_0004 - Multiple spaces and formatting', async ({ page }, testInfo) => {
  ...
  test('Neg_Fun_0005 - Mixed symbols and numbers', async ({ page }, testInfo) => {
  ...
  test('Neg_Fun_0006 - Time format with punctuation', async ({ page }, testInfo) => {
  ...
  test('Neg_Fun_0007 - Long paragraph', async ({ page }, testInfo) => {
    ...
    const expectOutput = 'இன்னைக்கு நான் ரொம்ப tired ஆக இருக்கேன் because காலைல இருந்து meeting மேல meeting போய்ட்டே இருந்தது, Zoom meeting, Teams meeting எல்லாமே back to back ஆக நடந்துச்சு, அதுக்கு மேல evening classes வேற இருந்துச்சு. அதனால correct ஆக சாப்பிட கூட முடியல'; 
  ...
  test('Neg_Fun_0008 - Symbols Handling ', async ({ page }, testInfo) => {
    const input = 'Ennoda maama velinaadula irunthu 130$ kittathatta 58000/=  enakku thanthaar';
    const expectOutput = 'என்னோட மாமா வெளிநாட்டுல இருந்து 13௦$ கிட்டத்தட்ட 58௦௦௦/= எனக்கு தந்தார்';
  ...
  test('Neg_Fun_0009 - Phone number sequence handling', async ({ page }, testInfo) => {
  ...
  test('Neg_Fun_0010 - abbreviation words with names/places', async ({ page }, testInfo) => {
